  ...
  for (rxnID = 0; rxnID < nreacts; rxnID++) {
    for (lcl_inst = 0; lcl_inst < nattempt[rxnID]; lcl_inst++) {
      ...
      if (glove_counter == onemol->natoms) {
        ...
        // NOTE: super simplified for Adrien and Geneveive
...
{
  // NOTE: MODs for Adrien + Geneveive for super-simple bond-react
  // major simplification = don't do any checks, let edge atom also be landlocked
